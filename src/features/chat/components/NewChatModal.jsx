...
const NewChatModal = ({ currentUser, onSelectUser, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[80vh] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border dark:border-slate-800 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 p-4">
          <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Новый чат</h3>
          <button
            onClick={onClose}
            className="inline-flex h-8 w-8 items-center justify-center rounded-full text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300"
            aria-label="Закрыть"
          >
        ...
        <div className="border-b border-slate-100 dark:border-slate-800 p-3">
          <input
            ...
            className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 px-4 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900/40"
            autoFocus
          />
        </div>
          ...
          {filtered.length > 0 ? (
            filtered.map((user) => (
              <button
                ...
                className="flex w-full items-center gap-3 border-b border-slate-50 dark:border-slate-800 p-3 text-left transition-colors hover:bg-blue-50 dark:hover:bg-slate-800"
              >
                <Avatar url={user.avatarUrl} displayName={user.displayName} colorKey={user.uid} size="md" />
